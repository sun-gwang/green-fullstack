...
create view `vw_member_with_sales` as (select a.`uid` as `직원아이디`,
									    b.`name` as `직원이름`,
                                        b.`pos` as `직급`,
                                        c.`name` as `부서명`,
                                        a.`year` as `매출년도`,
                                        a.`month` as `월`,
                                        a.`sale` as `매출액` from `sales` as a join `member` as b on a.uid = b.uid 
...
delimiter$$
create procedure proc_test4(in _pos varchar(10), out _count int)  # out=return 되는 반환값
begin
...
# 실습 5-9 프로시저 프로그래밍
delimiter $$
create procedure proc_test5(in __name varchar(10))
begin
